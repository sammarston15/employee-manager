...
export default class Timecard extends Component {
  ...
  render() {
    ...
    const timeMap = this.props.hours.map((day, i) => {
      ...
      return (
        <tr key={i}>
          <td>{moment(timeIn).tz("America/Boise").format("MM-DD-YYYY")}</td>
          <td>{moment(timeIn).tz("America/Boise").format("hh:mm A z")}</td>
          <td>
            {day.timeOut
              ? moment(timeOut).tz("America/Boise").format("hh:mm A z")
              : "-"}
          </td>
          <td>{day.timeOut ? `${hh}.${mm} hours` : "-"}</td>
        </tr>
      );
    });
